...
/**
 * Main panel for the user interface of a Boulder Slider game.
 * 
 * @author smkautz
 * @author tancreti
 */
public class BoardPanel extends JPanel {
	...
	/**
	 * The row location of the current boulder grab.
	 */
	private int grabbedRow;
	
	/**
	 * The col location of the current boulder grab.
	 */
	...
	public void paintComponent(Graphics g) {
		...
		// draw the boulders
		ArrayList<Boulder> boulders = board.getBoulders();
	...
	private class MyMouseListener implements MouseListener {
		...
		public void mousePressed(MouseEvent event) {
			if (!board.isGameOver()) {
				grabbedRow = event.getY() / GameMain.SIZE;
				grabbedCol = event.getX() / GameMain.SIZE;
				board.grabBoulderAt(grabbedRow, grabbedCol);
			}
			repaint();
		}
	...
	private class MyMouseMotionListener implements MouseMotionListener {
		...
		public void mouseDragged(MouseEvent e) {
			if (!board.isGameOver()) {
				...
				int curRow = grabbedRow;
				int curCol = grabbedCol;
